...
function initialize() {
    var items = ["hasAnswered", "ch01", "ch02"];
    var item_val = ["", true, true];
    for (var i in items) {
...
function random_func() {
    ...
    for (let i = 0; i < toBeAnswered.size && i < max_show_per; i++){
        let flr = Math.max(toBeAnswered.size, max_show_per);
        let randNum = Math.floor(Math.random() * flr);
...
function newList() {
    ...
    if (answering.size === 0) {
        ...
        for (let ii of answering) {
            let it = localdata[ii];
            createSheet(ii, it.stem, it.type, it.answers.split('/'), it.correctAnswer, it.analysis);
        }
    }
}
...
function createSheet(idx, stem, ty, choices, ans, ana) {
    ...
    let theHTML = `<div class="sheetAnswer" id="the_sheet_idx_${idx}">
                    <div class="sheet_title"><span>${multichoice+stem}</span></div>
    ...
    // show analysis
    document.getElementById("ans_for_"+idx).addEventListener('click', function() {
    ...
    if (ty === "单选题" || ty === "多选题" || ty === "判断题") {
        ...
        // check answer
        the_sheet_div.children[2].children[0].addEventListener("click", function() { // submit
            ...
            for (let j in c_ans) {
                if (c_ans[j] === true) {
                    the_sheet_div.children[1].children[j].children[1].style['background-color'] = '#82f6a194';
                }
            }
